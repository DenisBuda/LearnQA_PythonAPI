...
def assert_valid_schema(data, schema_file_name, schema_file_path):
    """ Checks whether the given data matches the schema """

    schema = _load_json_schema(schema_file_name, schema_file_path)
    return validate(data, schema)

def _load_json_schema(filename, file_path):
    """ Loads the given schema file """

    #relative_path = join('schemas', filename)
    relative_path = join(file_path, filename)
